
public class Const {

	public const int FLOOR = 0;
	public const int WALL = 1;
	public const int BLOKENWALL = 2;
	public const int BOMB = 3;
	public const int EXPLOSION= 4;
	public const int BOMBITEM= 5;
	public const int FIREITEM= 6;
	public const int HEARTITEM= 7;
	public const int SHOESITEM= 8;
	public const int BOMBITEMBLOCK= 9;
	public const int FIREITEMBLOCK= 10;
	public const int HEARTITEMBLOCK= 11;
	public const int SHOESITEMBLOCK= 12;
	public const int PLAYER1= 9;


	public const float BOMBTIMER = 3.0f;
	public const int WIDTH= 15;
	public const int HEIGHT= 13;

}
